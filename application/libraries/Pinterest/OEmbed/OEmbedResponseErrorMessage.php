...
/**
 * Concrete class defining basic necessary methods for error message creation and manipulation.
 * Represents concrete error OEmbed response message.
 * 
 ...
 * @file
 */
class OEmbedResponseErrorMessage extends OEmbedAbstractResponseMessage{
    ...
    public function __construct( $keys_and_values_array ) {
        if (is_array($keys_and_values_array)  ){
            $this->response_error_message_data = $keys_and_values_array;
        }
    }
    ...
    public function addData($keys_and_values_array) {
        if ( is_array($keys_and_values_array )  ){
            foreach ($keys_and_values_array as $key => $value) {
                $this->response_error_message_data[$key] = $value;
            }
        }
    }
    ...
    public function getData() {
        return $this->response_error_message_data;
    }
